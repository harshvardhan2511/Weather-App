...
//import icons
import { IoMdSunny, IoMdRainy, IoMdCloudy, IoMdSnow, IoMdThunderstorm, IoMdSearch } from
  'react-icons/io'

import { BsCloudHaze2Fill, BsCloudDrizzleFill, BsEye, BsWater, BsThermometer, BsWind, BsSnow } from
  'react-icons/bs'
...
// API
const APIkey = '90d785120a1161de9eca16e58b8c11cb';
...
const App = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if(input.value === ''){
      // set animate to true
      setAnimate(true);
      //after 500ms set animate to false again
      setTimeout(() => {
        setAnimate(false);
      }, 500);
  ...
  if (!data) {
    return <div className='w-full h-screen bg-gradientBg bg-no-repeat bg-cover bg-center flex flex-col justify-center items-center'>
      <div>
        <ImSpinner8 className='text5x1 animate-spin text-white' />
      </div>
    </div>
  }
  ...
  switch (data.weather[0].main) {
    case 'Clouds':
      icon = <IoMdCloudy className='text-[#dadadae1]' />
      break;
    case 'Haze':
      icon = <BsCloudHaze2Fill />;
      break;
    case 'Rain':
      icon = <IoMdRainy className='text-[#3f5eae]'/>;
      break;
    case 'Snow':
      icon = <IoMdSnow className='text-[#f9fbfb]'/>;
      break;
    case 'Clear':
      icon = <IoMdSunny className='text-[#eefc9f]' />;
      break;
    case 'Drizzle':
      icon = <BsCloudDrizzleFill />;
      break;
    case 'Thunder':
      icon = <IoMdThunderstorm className='text-[#1b3985]'/>;
      break;
  }
